...
def parse_args():
    ...
    parser.add_argument('return_output', default = None, help='return output')
    ## End of Custom Code
...
def main():
    ...
    # 🔹 Debug: Check if video samples are loaded correctly
    # try:
    #     for i in range(5):  # Try loading first 5 samples
    #         data = next(data_iter)  # Force-load sample
    #         print(f"✅ Loaded Sample {i + 1}: {data['imgs'].shape}")  # Print shape of images
    # except StopIteration:
    #     print("⚠️ No data loaded. Dataset might be empty or incorrectly formatted.")
    # Debug: Print dataset details before running inference
    # print(f"Total dataset length: {len(dataset)}")
    # print("First 5 video paths in dataset:")
    # for i in range(min(5, len(dataset))):
    #     print(f"Video {i + 1}: {dataset.video_infos[i]['filename']}")
    ...
    if args.tensorrt:
        ...
        print("Raw Model Outputs:", outputs)  # Debugging
    ...
    if rank == 0:
        ...
        if output_config.get('out', None):
            ### Custom Code ###
            # out_file = output_config["out"]
            # temp_file = out_file + ".tmp"  # Temporary file for atomic writing
            #
            # with open(temp_file, "wb") as f:
            #     pickle.dump(outputs, f)

            # # ✅ Move file atomically after writing completes
            # shutil.move(temp_file, out_file)
            # print(f"🎯 Features saved successfully at {out_file}")
            if args.return_output:
                outputs = torch.tensor(outputs)  # Convert to Tensor if needed

                import sys
                import pickle

                # Debugging: Print to stderr to check execution
                print(f"✅ Returning extracted tensor. Tensor shape: {outputs.shape}", file=sys.stderr)

                # Ensure we write only binary pickle data
                sys.stdout.buffer.write(pickle.dumps(outputs))  # ✅ Only write once
                sys.stdout.flush()
                if args.return_video:
                    torch.save(outputs,'batch_features_videos.pt')
                else:
                    torch.save(outputs, "batch_features_lips.pt")

                # else:
                #     torch.save(outputs, "batch_features.pt")
                # if args.frames_only == 1:
                #     torch.save(outputs, "batch_features_videos.pt")
                # else:
                #     torch.save(outputs, "batch_features.pt")
