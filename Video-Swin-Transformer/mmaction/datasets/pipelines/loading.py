...
class SampleFrames:
    ...
    def _get_train_clips(self, num_frames):
        ...
        if avg_interval > 0:
            ...
            clip_offsets = np.zeros((self.num_clips, ), dtype=np.uint8)
    ...
    def _get_test_clips(self, num_frames):
        ...
        if num_frames > ori_clip_len - 1:
            base_offsets = np.arange(self.num_clips) * avg_interval
            clip_offsets = (base_offsets + avg_interval / 2.0).astype(np.uint8)
    ...
    def __call__(self, results):
        ...
        results['frame_inds'] = frame_inds.astype(np.uint8)
...
class SampleAVAFrames(SampleFrames):
    ...
    def __call__(self, results):
        ...
        results['frame_inds'] = np.array(frame_inds, dtype=np.uint8)
        results['clip_len'] = self.clip_len
        results['frame_interval'] = self.frame_interval
        results['num_clips'] = 1
...
class SampleProposalFrames(SampleFrames):
    ...
    def _get_train_indices(valid_length, num_segments):
        ...
        if avg_interval > 0:
            ...
            offsets = np.zeros((num_segments, ), dtype=np.uint8)
    ...
    def _get_val_indices(valid_length, num_segments):
        ...
        if valid_length >= num_segments:
            ...
            base_offsets = np.arange(num_segments) * avg_interval
            offsets = (base_offsets + avg_interval / 2.0).astype(np.uint8)
        else:
            offsets = np.zeros((num_segments, ), dtype=np.uint8)
    ...
    def _get_test_clips(self, num_frames):
        ...
        ori_clip_len = self.clip_len * self.frame_interval
        return np.arange(
            0, num_frames - ori_clip_len, self.test_interval, dtype=np.uint8)
    ...
    def __call__(self, results):
        ...
        results['frame_inds'] = np.array(frame_inds).astype(np.uint8)
        results['clip_len'] = self.clip_len
        results['frame_interval'] = self.frame_interval
        results['num_clips'] = (
    ...
    def __call__(self, results):
        ...
        for packet in container.demux(stream):
            for frame in packet.decode():
                ...
                height = frame.height
                width = frame.width
                mv = np.zeros((height, width, 2), dtype=np.uint8)
