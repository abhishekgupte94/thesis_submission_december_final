...
#
# [SUPER-BATCH PATCH]
#   - ONLY ADD: audio96_rel, audio2048_rel, video_rel (lists of strings in batch)
...
class SegmentDataset(Dataset):
            ...
            out.append(
                {
                    ...
                    "seg_idx": int(seg_idx),
                    "T_video": int(T_video),
                    "a96_rel": str(Path(a96).resolve().relative_to(self.batch_dir.resolve())),
                    "a2048_rel": str(Path(a2048).resolve().relative_to(self.batch_dir.resolve())),
                    "v_rel": str(Path(v_pt).resolve().relative_to(self.batch_dir.resolve())),
                }
            )
        cache_path.write_text(json.dumps(out))

    # ------------------------------------------------------------
    def _acquire_lock(self, lock_path: Path, timeout_s: float = 60.0) -> bool:
        t0 = time.time()
        while True:
            try:
    ...
    def _build_index(self) -> None:
        ...
        try:
            ...
            # ============================================================
            # [KEPT] load cache if present
            # ============================================================
            if cache_path.exists() and self._try_load_index_cache(cache_path):
                return
    ...
    def __getitem__(self, i: int) -> Dict[str, object]:
        ...
        # ============================================================
        # [ADDED][SUPER-BATCH] Relative paths for path-based evaluators
        # RELATIVE to: <offline_root>/<batch_name>/
        # ============================================================
        audio96_rel = str(a96_pt.resolve().relative_to(self.batch_dir.resolve()))
        audio2048_rel = str(a2048_pt.resolve().relative_to(self.batch_dir.resolve()))
        video_rel = str(v_pt.resolve().relative_to(self.batch_dir.resolve()))
        ...
        return {
            ...
            "y": y,
            # ============================================================
            # [ADDED][SUPER-BATCH] rel-path keys (strings)
            # ============================================================
            "audio96_rel": audio96_rel,
            "audio2048_rel": audio2048_rel,
            "video_rel": video_rel,
        }
...
def collate_segments_pad(items: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    # ============================================================
    # [ADDED][SUPER-BATCH] carry rel paths through collate (lists)
    # ============================================================
    audio96_rel = [str(it.get("audio96_rel", "")) for it in items]
    audio2048_rel = [str(it.get("audio2048_rel", "")) for it in items]
    video_rel = [str(it.get("video_rel", "")) for it in items]
    ...
    return {
        ...
        "y_onehot": y_onehot,
        # ============================================================
        # [ADDED][SUPER-BATCH] rel-path fields
        # ============================================================
        "audio96_rel": audio96_rel,
        "audio2048_rel": audio2048_rel,
        "video_rel": video_rel,
...
class SegmentDataModuleCombined(pl.LightningDataModule):
