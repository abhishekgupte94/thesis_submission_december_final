...
def main():
    ap = argparse.ArgumentParser(description="Check if tokens vary across videos (no hardcoding).")
    ...
    # Align to the minimum shape over all videos (no hardcoding)
    ...
    # 1) Mean feature variance across videos: var_over_videos(N) → (L,D) → mean over D => (L,)
    var_over_videos = np.var(X_tokens, axis=0)            # (L_used, D)
    token_var_mean = var_over_videos.mean(axis=1)         # (L_used,)

    # 2) Norm-based token energy per video then variance across videos
