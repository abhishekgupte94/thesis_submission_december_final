...
#
# Output CSV columns:
#   clip_id, seg_idx, audio_rel, video_rel
...
def build_manifest(
    ...
    *,
    ...
    for clip_id, seg_idx, a_pt in iter_audio_pt_files(audio_root):
        ...
        yield clip_id, seg_idx, a_rel, v_rel
...
def main() -> None:
    ...
    with out_path.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["clip_id", "seg_idx", "audio_rel", "video_rel"])
        for clip_id, seg_idx, a_rel, v_rel in build_manifest(
            batch_dir,
            audio_dirname=args.audio_dirname,
            video_dirname=args.video_dirname,
            strict=bool(args.strict),
        ):
            w.writerow([clip_id, seg_idx, a_rel, v_rel])
            rows += 1
