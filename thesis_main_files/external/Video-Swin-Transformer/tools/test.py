...
def parse_args():
    ...
    parser.add_argument(
        ...
        'the evaluation_for_detection_model speed')
    parser.add_argument(
        '--eval',
        type=str,
        nargs='+',
...
def inference_tensorrt(ckpt_path, distributed, data_loader, batch_size):
    ...
    assert not distributed, \
        'TensorRT engine evaluation_for_detection_model only supports single gpu mode.'
...
def inference_onnx(ckpt_path, distributed, data_loader, batch_size):
    ...
    For now, multi-gpu mode and dynamic tensor shape are not supported.
    """
    assert not distributed, 'ONNX evaluation_for_detection_model only supports single gpu mode.'
