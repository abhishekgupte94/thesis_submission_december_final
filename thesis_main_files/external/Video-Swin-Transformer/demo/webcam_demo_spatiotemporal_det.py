...
class StdetPredictor:
    ...
    def predict(self, task):
        """Spatio-temporval Action Detection model evaluation_for_detection_model."""
        # No need to do evaluation_for_detection_model if no one in keyframe
...
class ClipHelper:
    ...
    def __next__(self):
        ...
        if not was_read:
            ...
            # in the task.processed_frames, so no need to model evaluation_for_detection_model
...
def main(args):
    ...
    try:
        ...
        for able_to_read, task in clip_helper:
            ...
            logger.debug('Main thread evaluation_for_detection_model time '
