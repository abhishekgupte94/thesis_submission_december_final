...
def parse_args():
    ...
    parser.add_argument(
        '--evaluation_for_detection_model-fps',
        type=int,
        default=4,
        help='Set upper bound FPS value of model evaluation_for_detection_model')
    parser.add_argument(
        '--cfg-options',
        nargs='+',
        action=DictAction,
    ...
    assert args.drawing_fps >= 0 and args.inference_fps >= 0, \
        'upper bound FPS value of drawing and evaluation_for_detection_model should be set as ' \
...
def inference():
    ...
    while True:
        ...
        if inference_fps > 0:
            # add a limiter for actual evaluation_for_detection_model fps <= inference_fps
