...
class EnsembleModelWrapper(nn.Module):
    ...
    def forward(self, batch) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(out_npv, dict): out_npv = out_npv["logits"]
        if isinstance(out_lavdf, dict): out_lavdf = out_lavdf["logits"]
        ...
        if self.inputs_are_probs:
            ...
            pA = torch.sigmoid(out_npv)
            pB = torch.sigmoid(out_lavdf)
