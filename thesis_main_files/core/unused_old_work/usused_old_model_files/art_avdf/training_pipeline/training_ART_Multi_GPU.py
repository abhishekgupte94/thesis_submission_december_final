...
from thesis_main_files.core.unused_old_work.usused_old_model_files.art_avdf.encoders.new_encoder_for_ssl.alignment_pipeline_gpu import GPUOptimizedAlignment, SelfSupervisedAVLoss
log_dir = f"runs/ssl_ddp_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
class TrainingPipeline:
    def __init__(self, dataset, batch_size, learning_rate, num_epochs, device, feature_processor, output_txt_path, local_rank):
        ...
        if dist.is_initialized():
            ra = dist.get_rank()
            try:
                mvit_dev = feature_processor.device
                ast_dev = feature_processor.device
            except Exception:
