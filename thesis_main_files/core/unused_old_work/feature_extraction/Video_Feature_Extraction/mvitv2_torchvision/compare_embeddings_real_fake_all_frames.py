...
- Compares video with DTW (cosine distance)
...
def main():
    ...
    if not files:
        print(f"No video under {root}/{{eval_real,eval_fake}}"); sys.exit(1)
    names = [p.stem for p in files]
    if args.verbose:
        print(f"Found {len(files)} video → real={labels.count('real')} fake={labels.count('fake')}")
