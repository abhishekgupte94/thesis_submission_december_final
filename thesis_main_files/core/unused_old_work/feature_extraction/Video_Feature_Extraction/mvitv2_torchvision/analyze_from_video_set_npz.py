...
def compute_variation(X: np.ndarray, video_keys: list[str]):
    ...
    # Variance across video for each (t, d), then summarize:
    ...
    # Token-wise L2 energy per video → variance across video
    token_energy = np.linalg.norm(X, axis=2)   # (N, L)
    temporal_var_energy = np.var(token_energy, axis=0)  # (L,)

    # Feature-wise L2 energy over time per video → variance across video
    ...
    # Pairwise cosine similarity between video
...
def maybe_plot_temporal(temporal_var_mean: np.ndarray, temporal_var_energy: np.ndarray, out_png: Path):
    ...
    plt.ylabel("Variance across video")
    plt.title("Temporal variation across video")
    plt.legend()
    plt.tight_layout()
...
def maybe_plot_feature(feature_var_mean: np.ndarray, feature_var_energy: np.ndarray, out_png: Path):
    ...
    plt.ylabel("Variance across video")
    plt.title("Feature-dimension variation across video")
    plt.legend()
    plt.tight_layout()
    out_png.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_png)
...
def main():
    parser = argparse.ArgumentParser(description="Analyze temporal and feature variation across video in a .npz")
    ...
    print(f"[INFO] Found {len(video_keys)} video: {video_keys}")
