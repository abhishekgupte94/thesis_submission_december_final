...
#           x = x + self.absolute_pos_embed
        ...
        x = self.norm(x)  # B L C
# <<< [MODIFIED BY Abhishek Gupte on 2025-11-22 [YYYY-MM-DD]]
# Reason: to create an additional token sequence
        token_sequence = x
# <<< End of Modification
        ...
        x = torch.flatten(x, 1)
# <<< [MODIFIED BY Abhishek Gupte on 2025-11-22 [YYYY-MM-DD]]
# Reason: to return an additional token sequence
        # return x
        return x, token_sequence
# <<< End of Modification
...
# Reason: to return an additional token sequence
        ...
        x, token_sequence = self.forward_features(x)
# <<< End of Modification
        x = self.head(x)
